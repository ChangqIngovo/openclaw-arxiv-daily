...
电脑需要开机、联网、保持清醒，且 Gateway 正在运行。休眠期间无法推送；当日醒来后可处理当时的前一日，跨日不补历史。每次窗口都是配置时区前一个自然日的 `[00:00, 次日 00:00)`，依据 arXiv API `published`，不依据 `updated`、公告批次或本机抓取时间。晚入库的论文可能因此漏过，不扩大日期窗口补发。
