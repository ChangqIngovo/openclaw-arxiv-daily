...
- Category subscriptions use validated official identifiers, API `cat:` queries and exact metadata matching including cross-lists. Keep category aliases canonical, keyword matching separate, and mixed priorities deduplicated. Update identifiers from the official taxonomy; never accept arbitrary query syntax.
- Every new, test, retry and resumed delivery is restricted to the previous civil day in the effective timezone, using first submission time. The default system mode follows the computer while running; an explicit IANA zone stays fixed. Scheduling, commands, paper dates and window guards must agree, including DST and timezone changes during delivery. Do not widen the range to fill empty days.
