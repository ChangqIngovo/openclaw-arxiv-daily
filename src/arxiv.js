  ...
  async refresh(topics, signal, window = previousDayWindow(this.clock(), this.config.timeZone)) {
    ...
    for (const batch of topicBatches(topics)) {
      ...
      do {
        ...
        const response = await this.fetchImpl(url, { signal: requestSignal, headers: { Accept: 'application/atom+xml', 'User-Agent': 'openclaw-arxiv-daily/0.5.0 (OpenClaw literature digest)' } });
