  ...
  async refresh(topics, signal) {
    ...
    for (const batch of topicBatches(topics)) {
      ...
      do {
        ...
        const response = await this.fetchImpl(url, { signal: requestSignal, headers: { Accept: 'application/atom+xml', 'User-Agent': 'openclaw-arxiv-daily/0.1.1 (OpenClaw literature digest)' } });
